Feature: Revealing a Secret's plaintext
  ...
  The vault_reveal MCP Tool returns the plaintext of a Secret's Private Blob to the
  ...
  Scenario: Reveal sensitivity=low Secret via slash command without OOB Confirmation
    ...
    When the MCP Adapter invokes vault_reveal with handle "vault://acme-backend/note/architecture-notes"
  ...
  Scenario: Reveal sensitivity=medium Secret via slash command without OOB Confirmation
    ...
    And the client sets operator_confirmation with slash_command=true and oob_ack=false
    When the MCP Adapter invokes vault_reveal with handle "vault://acme-backend/token/deploy-token-prod"
  ...
  Scenario: Reveal sensitivity=high Secret requires both slash_command and oob_ack
    ...
    And the client sets operator_confirmation with slash_command=true and oob_ack=false
    When the MCP Adapter invokes vault_reveal with handle "vault://acme-backend/password/db-admin"
  ...
  Scenario: LLM-initiated reveal without a slash command is denied regardless of sensitivity
    Given the LLM constructs a vault_reveal call with handle "vault://acme-backend/token/deploy-token-prod"
    ...
    And the error message states that vault_reveal requires slash_command=true
    And no decryption is performed
  ...
  Scenario: Reveal-only category note rejects proxy tool invocations but supports reveal
    ...
    When the LLM calls vault_ssh_exec with handle "vault://acme-backend/note/architecture-notes"
  ...
  Scenario: Reveal succeeds via signed_config_flag JWT for non-Claude client
    ...
    When vault_reveal is called with operator_confirmation { slash_command: false, oob_ack: false, signed_config_flag: <jwt> }
