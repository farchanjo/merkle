Feature: BM25-Ranked Full-Text Search over Public Metadata
  ...
  The vault.search MCP Tool and the fts_query parameter of vault.list
  ...
  Scenario: Name-match ranks above description-match for the same query term
    ...
    When the operator calls vault.search with query "github"
  ...
  Scenario: Weighted BM25 prevents TF-stuffing from overriding name match
    ...
    When the operator calls vault.search with query "deploy"
  ...
  Scenario: Pagination preserves rank order across pages
    ...
    When the operator calls vault.search with query "acme" limit 5 offset 0
    ...
    When the operator calls vault.search with query "acme" limit 5 offset 5
  ...
  Scenario: Highlight snippets are present and reference only public fields
    ...
    When the operator calls vault.search with query "bastion"
  ...
  Scenario: Private fields never appear in highlights or search results
    ...
    When the operator calls vault.search with query "SUPERSECRET"
    Then the response contains zero results
    When the operator calls vault.search with query "prod"
  ...
  Scenario: FTS5 index reflects updated description after rotate
    ...
    And the operator calls vault.search with query "initial"
    Then the Secret "vault://acme-backend/token/old-token" does not appear in results
    When the operator calls vault.search with query "oauth"
  ...
  Scenario: Porter stemming resolves inflected query terms
    ...
    When the operator calls vault.search with query "authenticat"
    Then the Secret "vault://acme-backend/token/auth-service-token" appears in results
    When the operator calls vault.search with query "authoriz"
    Then the Secret "vault://acme-backend/token/auth-service-token" appears in results

  Scenario: Doctor check validates FTS5 schema column list
    When the operator calls vault.doctor with check "fts5_schema"
