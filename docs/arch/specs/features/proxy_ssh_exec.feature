Feature: Executing a remote command via the SSH proxy tool
  ...
  The vault_ssh_exec MCP Tool invokes a command on a remote host using the SSH Bridge
  ...
  Scenario: Execute uptime using the Handle without the private key crossing the MCP transport
    When the LLM calls vault_ssh_exec with handle "vault://acme-backend/ssh/db-prod" and command "uptime"
  ...
  Scenario: Execution fails when the Handle resolves to a non-ssh category
    ...
    When the LLM calls vault_ssh_exec with handle "vault://acme-backend/password/db-admin" and command "whoami"
    Then the Vault Agent rejects the request with error "category_mismatch"
    And the error message states that vault_ssh_exec requires category "ssh" but received category "password"
  ...
  Scenario: Execution fails when the rate limit for use_token_resolves is exceeded
    ...
    When the LLM calls vault_ssh_exec with handle "vault://acme-backend/ssh/db-prod" and command "df -h"
  ...
  Scenario: Support jump-host chaining via jump_host_handle field
    ...
    When the LLM calls vault_ssh_exec with handle "vault://acme-backend/ssh/db-prod" and command "hostname"
  ...
  Scenario: Operator Confirmation is NOT required for proxy execution at sensitivity medium or below
    ...
    When the LLM calls vault_ssh_exec with handle "vault://acme-backend/ssh/db-prod" and command "ps aux"
