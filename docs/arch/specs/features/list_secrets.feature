Feature: Listing Secrets in a Namespace
  ...
  The vault.list MCP Tool returns Public Metadata for all Secrets visible to the
  ...
  Scenario: Filter by Category returns only matching Secrets
    When the operator calls vault.list with filter "category=ssh"
  ...
  Scenario: Filter by Tag key:value query returns only matching Secrets
    When the operator calls vault.list with filter "tag=env:prod"
  ...
  Scenario: Free-text FTS5 search returns ranked results
    ...
    When the operator calls vault.list with query "bastion"
  ...
  Scenario: List response excludes private material in all response shapes
    When the operator calls vault.list with filter "category=password"
  ...
  Scenario: Pagination by created_at desc with cursor returns correct page
    ...
    When the operator calls vault.list with "limit=3" and no cursor
    ...
    When the operator calls vault.list with the returned next_cursor and "limit=3"
