...
Daemon spawns: backup scheduler (anacron, ADR-0010), chain verifier, tempfile reaper, idle-relock supervisor (`[security] session_idle_timeout_secs`, default **1800s** — note the worker `idle_relock_task` is currently a stub that only awaits shutdown; `idle_lock_timeout_secs` is a separate `Option<u64>` per-namespace-policy override that defaults to `None`), Prometheus server, socket server. Shutdown drains with a 30s hard timeout. **MCP:** `Claude Code → rmcp stdio → MerkleMcpServer → CompanionSocketClient → daemon`. The MCP adapter exposes **30 tools** (`lib.rs` `29 total` comment is stale) across identity/secrets/reveal/use_token/proxy/audit/backup/diagnostics, plus 4 prompts (`merkle-doctor`, `merkle-show`, `merkle-reveal`, `merkle-rollback`, ADR-0028). `SessionState` enforces at-most-one bind per session via two-phase `commit_binding()` (ADR-0026).
...
2. **Integration** — per-crate `tests/`, real or mock adapters (e.g. `merkle-application/tests/use_cases.rs`, `merkle-domain-audit-compliance/tests/chain_integrity.rs`, proptests in policy/backup). Covered by `cargo test --workspace`.
...
7. **UseToken is one-shot.** Resolving a `vault.use` token over the socket consumes it permanently; there's no re-issue without another `vault.use`.
